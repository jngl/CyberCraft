...
namespace ck
{
    ...
    public:
        ...
        void processKeyDown(cp::Key key){
    ...
    protected:
        void createOneFrameAction(std::unique_ptr<Command> command, cp::Key defaultKey){
            m_oneFrameAction.insert({defaultKey, std::move(command)});
        }

        void createMultiFrameAction(std::unique_ptr<Command> command, cp::Key defaultKey){
            m_multiFrameAction.insert({defaultKey, std::move(command)});
        }

    private:
        std::unordered_map<cp::Key, std::unique_ptr<Command>> m_oneFrameAction;
        std::unordered_map<cp::Key, std::unique_ptr<Command>> m_multiFrameAction;
